...
boru-driven command-line client **and** interactive REPL for the McuCountdown
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mcu-countdown-cli)
make build

# 2. See usage (words, entities, env vars)
./mcu-countdown-cli --help

# 3. Provide credentials once, via the environment
export MCU_COUNTDOWN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./mcu-countdown-cli load 1 api            # {id:1} shorthand
./mcu-countdown-cli load '{id:1}' api       # explicit match map

# 5. Override the API base URL for a single call
MCU_COUNTDOWN_BASE=https://api.example.com ./mcu-countdown-cli load 1 api

# 6. No arguments -> interactive REPL
./mcu-countdown-cli
mcu-countdown> load 1 api
mcu-countdown> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mcu-countdown-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mcu-countdown-cli load 1 api
   ```
...
```sh
./mcu-countdown-cli load 1 api          # scalar shorthand for {id:1}
./mcu-countdown-cli load '{id:1}' api     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `mcu-countdown>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mcu-countdown-cli`. |
...
(`./mcu-countdown-cli <expr>`) and interactively (the REPL), and expressions compose the
