// Command McuCountdown-cli is a CLI + REPL for the McuCountdown SDK,
...
const prompt = "mcu-countdown"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "api batman dcn star_war"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MCU_COUNTDOWN_APIKEY (api key), MCU_COUNTDOWN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MCU_COUNTDOWN_APIKEY carries the API key and
	// MCU_COUNTDOWN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MCU_COUNTDOWN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MCU_COUNTDOWN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMcuCountdownSDK(opts)
