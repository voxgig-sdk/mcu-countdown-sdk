...
class FtCtrl extends McuCountdownControl
{
...
class FtClient
{
    ...
    private McuCountdownUtility $utility;

    public function __construct(McuCountdownUtility $utility, array $options)
    {
    ...
    public function get_utility(): McuCountdownUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (McuCountdownContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public McuCountdownContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (McuCountdownContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = McuCountdownConfig::make_feature($name);
    ...
    private static function buildUrl(McuCountdownSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof McuCountdownError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new McuCountdownResponse($fetched);
        }
    ...
    private function fail(McuCountdownContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (McuCountdownContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
