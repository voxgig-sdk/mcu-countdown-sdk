...
class DcnEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = McuCountdownSDK::test(null, null);
        $ent = $testsdk->Dcn(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = dcn_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "dcn." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $dcn_ref01_data = null;
        if (count($dcn_ref01_data_raw) > 0) {
            $dcn_ref01_data = Helpers::to_map($dcn_ref01_data_raw[0][1]);
        }

        // LOAD
        $dcn_ref01_ent = $client->Dcn(null);
        $dcn_ref01_match_dt0 = [
            "id" => $dcn_ref01_data["id"],
        ];
        [$dcn_ref01_data_dt0_loaded, $err] = $dcn_ref01_ent->load($dcn_ref01_match_dt0, null);
        $this->assertNull($err);
...
function dcn_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/dcn/DcnTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["dcn01", "dcn02", "dcn03"] as $k) {
