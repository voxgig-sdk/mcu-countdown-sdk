...
// McuCountdown SDK StarWar entity
...
class StarWarEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new StarWarEntity($this->_client, $opts);
    }

    /**
     * @param StarWar|array $args StarWar data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = McuCountdownHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return StarWar|array The current StarWar data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of StarWar fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = McuCountdownHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of StarWar fields).
     */
    ...
    /**
     * Load a single StarWar.
     *
     * @param StarWarLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed StarWarLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return StarWar|array The loaded StarWar as an assoc-array at the
     *   SDK boundary; throws McuCountdownError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = McuCountdownHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
