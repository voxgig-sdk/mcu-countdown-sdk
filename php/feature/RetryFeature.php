...
// McuCountdown SDK retry feature
...
class McuCountdownRetryFeature extends McuCountdownBaseFeature
{
    ...
    public function init(McuCountdownContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (McuCountdownContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(McuCountdownContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(McuCountdownContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
