...
local McuCountdownSDK = {}
McuCountdownSDK.__index = McuCountdownSDK
...
McuCountdownSDK._make_feature = _make_feature


function McuCountdownSDK.new(options)
  local self = setmetatable({}, McuCountdownSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function McuCountdownSDK:options_map()
...
function McuCountdownSDK:get_utility()
  return Utility.copy(self._utility)
end


function McuCountdownSDK:get_root_ctx()
  return self._rootctx
end


function McuCountdownSDK:prepare(fetchargs)
...
function McuCountdownSDK:direct(fetchargs)
...
-- by passing it.
function McuCountdownSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function McuCountdownSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "McuCountdownSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function McuCountdownSDK:Batman(data)
  ...
  if data == nil then
    if self._batman == nil then
      self._batman = EntityMod.new(self, nil)
    end
    return self._batman
  end
  return EntityMod.new(self, data)
...
function McuCountdownSDK:Dcn(data)
  ...
  if data == nil then
    if self._dcn == nil then
      self._dcn = EntityMod.new(self, nil)
    end
    return self._dcn
  end
  return EntityMod.new(self, data)
...
function McuCountdownSDK:StarWar(data)
  ...
  if data == nil then
    if self._star_war == nil then
      self._star_war = EntityMod.new(self, nil)
    end
...
function McuCountdownSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = McuCountdownSDK.new(sdkopts)
  sdk.mode = "test"
