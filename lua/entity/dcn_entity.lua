...
local DcnEntity = {}
DcnEntity.__index = DcnEntity


function DcnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DcnEntity)
  self._name = "dcn"
  self._client = client
...
function DcnEntity:get_name()
  return self._name
end


function DcnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DcnEntity.new(self._client, opts)
end
...
function DcnEntity:mark_deleted()
  self._deleted = true
end


function DcnEntity:deleted()
  return true == self._deleted
end


function DcnEntity:data_set(args)
  if args ~= nil then
...
function DcnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DcnEntity:match_set(args)
  if args ~= nil then
...
function DcnEntity:stream(action, args, callopts)
...
function DcnEntity:_run_op(ctx, post_done)
  local utility = self._utility
