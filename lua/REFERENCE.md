# McuCountdown Lua SDK Reference

Complete API reference for the McuCountdown Lua SDK.
...
```lua
local result, err = client:Batman(nil):load({ id = "batman_id" }, nil)
```
...
```lua
local result, err = client:Dcn(nil):load({ id = "dcn_id" }, nil)
```
...
```lua
local result, err = client:StarWar(nil):load({ id = "star_war_id" }, nil)
```
