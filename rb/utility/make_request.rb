...
module McuCountdownUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = McuCountdownResponse.new({})
    ...
    if fetch_err
      ...
      response = McuCountdownResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = McuCountdownResponse.new(fetched)
    else
