...
class StarWarEntity
  ...
  def make
    opts = @_entopts.dup
    StarWarEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = McuCountdownHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [StarWar, Hash] the current StarWar data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = McuCountdownHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of StarWar fields)
  def match_get
  ...
  #
  # @param reqmatch [StarWarLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.StarWar.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [StarWar, Hash] the loaded StarWar; raises McuCountdownError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = McuCountdownHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
