...
require_relative 'McuCountdown_types'
...
class McuCountdownSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = McuCountdownHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = McuCountdownHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, McuCountdownFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    McuCountdownUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = McuCountdownHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = McuCountdownHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = McuCountdownHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => McuCountdownError.new(
        "#{op}_allow",
        "McuCountdownSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = McuCountdownHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = McuCountdownError.new(
        "graphql_error", "McuCountdownSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = McuCountdownSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
