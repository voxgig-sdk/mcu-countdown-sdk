...
class BatmanEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MCU_COUNTDOWN_TEST_BATMAN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    batman_ref01_data_dt0_load_result = Helpers.to_map(batman_ref01_data_dt0_loaded.respond_to?(:data_get) ? batman_ref01_data_dt0_loaded.data_get : batman_ref01_data_dt0_loaded)
...
def batman_basic_setup(extra)
  ...
  entid_env_raw = ENV["MCU_COUNTDOWN_TEST_BATMAN_ENTID"]
