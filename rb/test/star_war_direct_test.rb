...
class StarWarDirectTest < Minitest::Test
  def test_direct_load_star_war
    setup = star_war_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-star_war", setup[:live] ? "live" : "unit")
    if _should_skip
...
def star_war_direct_setup(mockres)
