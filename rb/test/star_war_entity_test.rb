...
class StarWarEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MCU_COUNTDOWN_TEST_STAR_WAR_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    star_war_ref01_data_dt0_load_result = Helpers.to_map(star_war_ref01_data_dt0_loaded.respond_to?(:data_get) ? star_war_ref01_data_dt0_loaded.data_get : star_war_ref01_data_dt0_loaded)
...
def star_war_basic_setup(extra)
  ...
  entid_env_raw = ENV["MCU_COUNTDOWN_TEST_STAR_WAR_ENTID"]
