...
class BatmanDirectTest < Minitest::Test
  def test_direct_load_batman
    setup = batman_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-batman", setup[:live] ? "live" : "unit")
    if _should_skip
...
def batman_direct_setup(mockres)
