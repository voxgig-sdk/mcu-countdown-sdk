...
func TestBatmanEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		batmanRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.batman")))
		...
		batmanRef01DataDt0LoadResult := core.ToMapAny(entityData(batmanRef01DataDt0Loaded))
...
func batmanBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MCU_COUNTDOWN_TEST_BATMAN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MCU_COUNTDOWN_TEST_BATMAN_ENTID"])
