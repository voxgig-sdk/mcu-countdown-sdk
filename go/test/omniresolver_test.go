...
import (
	...
	omni "github.com/voxgig-sdk/mcu-countdown-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.McuCountdownError); is {
				out := map[string]any{
					"name":    "McuCountdownError",
...
func omniCtx(arg any, client *sdk.McuCountdownSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.McuCountdownSDK); is {
			client = live
