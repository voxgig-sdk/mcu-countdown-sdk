...
func (sdk *McuCountdownSDK) OptionsMap() map[string]any {
...
func (sdk *McuCountdownSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *McuCountdownSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *McuCountdownSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *McuCountdownSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *McuCountdownSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *McuCountdownSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("McuCountdownSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *McuCountdownSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *McuCountdownSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("McuCountdownSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *McuCountdownSDK {
	...
	sdk := NewMcuCountdownSDK(sdkopts)
	sdk.Mode = "test"
