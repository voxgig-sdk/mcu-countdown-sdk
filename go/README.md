# McuCountdown Golang SDK



The Golang SDK for the McuCountdown API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/mcu-countdown-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/mcu-countdown-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/mcu-countdown-sdk/go=../mcu-countdown-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *McuCountdownSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### McuCountdownSDK methods
...
### Entity interface (McuCountdownEntity)

All entities implement the `McuCountdownEntity` interface.
...
Create an instance: `batman := client.Batman(nil)`
...
Create an instance: `dcn := client.Dcn(nil)`
...
```
github.com/voxgig-sdk/mcu-countdown-sdk/go/
├── mcu-countdown.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/mcu-countdown-sdk/go`) re-exports everything needed
...
```go
api := client.Api(nil)
api.Load(map[string]any{"id": 1}, nil)
