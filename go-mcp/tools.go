...
type Args struct {
	Entity string         `json:"entity" jsonschema:"api | batman | dcn | star_war"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.McuCountdownSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "mcu-countdown_list",
		Description: "List records from McuCountdown. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "mcu-countdown_load",
		Description: "Load a single record from McuCountdown. " +
...
func runOp(client *sdk.McuCountdownSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.McuCountdownSDK, name string) (sdk.McuCountdownEntity, error) {
	switch strings.ToLower(name) {
	case "api":
		return client.Api(nil), nil
	case "batman":
		return client.Batman(nil), nil
	case "dcn":
		return client.Dcn(nil), nil
	case "star_war":
		return client.StarWar(nil), nil
