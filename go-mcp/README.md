...
MCP server exposing the McuCountdown SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `mcu-countdown_list` | `entity`, optional `query` map | First page of records as JSON |
| `mcu-countdown_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `mcu-countdown_list` and
`mcu-countdown_load` tools become available in new sessions.
