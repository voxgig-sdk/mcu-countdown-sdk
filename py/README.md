# McuCountdown Python SDK



The Python SDK for the McuCountdown API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/mcu-countdown-sdk/releases)) or
...
```python
client = McuCountdownSDK.test(testopts, sdkopts)
```
...
| `overview` | `str` | Brief overview/synopsis of the production |
| `poster_url` | `str` | URL to the poster image from TMDB |
| `release_date` | `str` | Release date in YYYY-MM-DD format |
| `title` | `str` | Title of the following production |
| `type` | `str` | Type of production |
...
```
py/
├── mcucountdown_sdk.py         -- Main SDK module
...
The main module (`mcucountdown_sdk`) exports the SDK class.
