...
class TestBatmanEntity:

    def test_should_create_instance(self):
        testsdk = McuCountdownSDK.test(None, None)
        ent = testsdk.Batman(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _batman_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "batman." + _op, "live" if _live else "unit")
            if _skip:
        ...
        batman_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(batman_ref01_data_dt0_loaded))
        assert batman_ref01_data_dt0_load_result is not None
        assert batman_ref01_data_dt0_load_result["id"] == batman_ref01_data["id"]



def _batman_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/batman/BatmanTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MCU_COUNTDOWN_TEST_BATMAN_ENTID"))
