...
class TestBatmanDirect:

    def test_should_direct_load_batman(self):
        setup = _batman_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-batman", "live" if setup["live"] else "unit")
        if _skip:
...
def _batman_direct_setup(mockres):
