...
describe('DcnEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'dcn.' + op, live)) return
    }
    ...
    let dcn_ref01_data = Object.values(setup.data.existing.dcn)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/dcn/DcnTestData.json')
