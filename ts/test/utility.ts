...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MCUCOUNTDOWN_TEST_LIVE ||
    'TRUE' === process.env.MCUCOUNTDOWN_TEST_OVERRIDE
  ) {
  ...
  m.MCUCOUNTDOWN_TEST_EXPLAIN = process.env.MCUCOUNTDOWN_TEST_EXPLAIN || m.MCUCOUNTDOWN_TEST_EXPLAIN
