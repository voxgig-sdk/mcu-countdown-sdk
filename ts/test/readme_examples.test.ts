...
//     (\`new McuCountdownSDK(...)\` / \`McuCountdownSDK.test(...)\` become
//     \`McuCountdownSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'McuCountdownSDK'
...
const TEST_SEED = {"entity":{"api":{"test01":{"id":"test01"}},"batman":{"test01":{"id":"test01"}},"dcn":{"test01":{"id":"test01"}},"star_war":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new McuCountdownSDK(...)\` and any
// \`McuCountdownSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+McuCountdownSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/McuCountdownSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
